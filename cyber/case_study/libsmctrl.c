...
static void setup_sm_control_11() {
	...
	cuGetExportTable((const void**)&tbl_base, &callback_funcs_id); // callback_funcs_id未导出表的uuid, 16个元素的数组，每个元素为8为的无符号整型
...
static int read_int_procfile(char* filename, uint64_t* out) {
	...
	read(fd, f_data, 18);
	close(fd);
	*out = strtoll(f_data, NULL, 16);  //parse string to longlong with base 16, stopping position is null
	return 0;
}
	...
	for (i = 0; i < max_gpcs; i++) {
		...
		*tpc_mask = 0;
		for (j = 0; j < num_tpc_per_gpc; j++) {
				...
				if ((1 << j) & gpc_tpc_mask)
					continue;
				*tpc_mask |= (1ull << vtpc_idx); // wrong? should be 1ull << j
				vtpc_idx++;
		}
	}
