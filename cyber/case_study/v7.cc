...
bool v7_component::Proc(const std::shared_ptr<Driver>& msg0, const std::shared_ptr<Driver>& msg1) {

    apollo::cyber::Time now_time = Clock::Now();
    double start_time = now_time.ToMillisecond();
    
    int sizeA = 256; // 第一矩阵的行数和第二矩阵的列数
    int sizeB = 256; // 第一矩阵的列数和第二矩阵的行数
    // 执行矩阵生成与相乘
...
//     AINFO << "v7" << " started kernel at " << (long long)start_kernel << " ms," << " ended kernel at " << (long long)end_kernel << " ms.";
