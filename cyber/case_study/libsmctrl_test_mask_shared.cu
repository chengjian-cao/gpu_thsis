...
__global__ void read_and_store_smid(uint8_t* smid_arr) {
  // 一个block只能被schedule到一个SM，只需要记录第一个thread对应的SM ID即可；
  if (threadIdx.x != 1)
    return;
  int smid;
  // 前线程所处的 SM 的 ID 被读取并存储到变量 smid 中; %0为占位符， %%smid为储存SM ID的特殊寄存器
...
#define NUM_BLOCKS 1200 //6


// 升序，a<b => a-b为负数，负数在前面，所以a在b前面；
...
int test_constrained_size_and_location(enum partitioning_type part_type) {
  int res;
  uint8_t *smids_native_d, *smids_native_h; //d -> device, h -> host
  ...
  SAFE(cudaDeviceGetAttribute(&num_sms, cudaDevAttrMultiProcessorCount, 0)); // 0为device ID
  if (res = libsmctrl_get_tpc_info_cuda(&num_tpcs, 0))   //使用cuDeviceAttribute()获取sm数目和sm_per_tpc,然后num_tpcs = sm / sm_per_tpc
    error(1, res, "Unable to get TPC configuration for test");
  sms_per_tpc = num_sms/num_tpcs;
  printf("[KONTON_LOG]: num_tpcs=%d, sms_per_tpcs=%d, .\n", num_tpcs, sms_per_tpc);


// ---------------------------------------------- without global mask ------------------------------------------
  printf(" ------------------------------------  with global mask -----------------------------------");
  ...
  for (int enabled_tpc = 0; enabled_tpc < num_tpcs && enabled_tpc < 128; enabled_tpc += 32) {
    ...
    // for(int i=0; i<NUM_BLOCKS; i++){
    //   printf("block=%d is scheduled to sm=%d \n", i, smids_partitioned_h[i]);
    // }
    ...
    // printf("(global mask) Total used SM number=%d.\n", uniq_partitioned);

    // Make sure it ran on the right TPC
    // Case 1: 大于1 小于0，说明mask没有成功; Case 2: > 32 * 2 +1, < 32 * 2,也说明没有成功 
    ...
    // asprintf  allocate string printf, 动态分配内存给需要写入的字符串指针
