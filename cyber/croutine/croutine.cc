...
// 初始化静态线程局部变量
thread_local cudaStream_t CRoutine::current_stream_ = nullptr;
...
RoutineState CRoutine::Resume(cudaStream_t stream) {
  ...
  stream_ = stream;  // 存储传递的流
  current_stream_ = stream_;  // 设置线程局部变量
  SwapContext(GetMainStack(), GetStack());
  current_stream_ = nullptr;  // 清除线程局部变量
...
cudaStream_t CRoutine::GetCurrentStream() {
