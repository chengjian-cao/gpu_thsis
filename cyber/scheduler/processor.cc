...
Processor::Processor() : stream_(nullptr){ 
  running_.store(true); 
    // 创建 CUDA 流
  cudaError_t err = cudaStreamCreate(&stream_);
  if (err != cudaSuccess) {
      AINFO << "Failed to create CUDA stream: " << cudaGetErrorString(err);
      running_.store(false);
  } else {
      AINFO << "Processor created with CUDA stream: " << stream_ ;
  } 
}
  ...
  while (cyber_likely(running_.load())) {
    if (cyber_likely(context_ != nullptr)) {
      ...
      if (croutine) {
        ...
        // 传递 Processor 的 CUDA 流给 CRoutine
        croutine->Resume(stream_);
        croutine->Release();
      } else {
